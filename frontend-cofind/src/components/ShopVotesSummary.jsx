...
export function VoteBarGroup({ title, titleEmoji, items, counts, maxCount, selectedKey, onSelect, isSubmitting }) {
  return (
...
export function BestForBar({ counts, selectedKeys = [], onToggle, isSubmitting }) {
  const maxCount = Math.max(1, ...BEST_FOR_META.map((item) => counts?.[item.key] || 0));
  ...
  return (
    ...
    <div className="flex flex-col sm:flex-row gap-4">
      ...
      <BestForBar
        ...
        selectedKeys={myVote?.best_for || []}
        onToggle={onToggleBestFor}
        isSubmitting={isSubmitting}
      />
    </div>
  );

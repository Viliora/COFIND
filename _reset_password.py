"""
Reset password user di cofind.db
Jalankan: python _reset_password.py --username NAMA --password PASSWORD_BARU
"""
import sqlite3, hashlib, secrets, argparse, os

DB = os.path.join(os.path.dirname(__file__), 'cofind.db')
...
row = cursor.execute("SELECT id, username, email FROM users WHERE username = ?", (args.username,)).fetchone()
if not row:
